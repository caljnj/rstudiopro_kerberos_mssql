...
# --- /etc/cont-init.d/rserver/startup.sh ---
...
# create the log dirs
# this could probably be done at runtime... but what the hell let's leave it here.

mkdir -p /var/log/rserver
chown rstudio-server:rstudio-server /var/log/rserver

mkdir -p /var/log/rstudio-launcher
chown rstudio-server:rstudio-server /var/log/rstudio-launcher
...
# --need to ensure that all these logs end up sent to stdoutmand into podman logs
...
# --not sure if this is going to work as we HAVE to exec...
